...
if($current_month > 8){
  $season_year = $current_year.'-'.(date('y', strtotime($current_year)) + 1);
}else{
  $season_year = ($current_year - 1).'-'.(date('y', strtotime($current_year)));
}
$headerbg = 'https://elitebasketballcircuit.com/wp-content/uploads/2022/11/ebc-stat-leaderboard-banner.jpeg'; ?>
  ...
  <div class="full_width_container small-12 medium-12 large-12 medium-padding-top">
    <div class="stat-header__wrap large-margin-bottom">
      ...
      <div class="stat-header__info">
        <h2 class="stat-header__heading">Stat Leaderboard <?php echo $season_year; ?></h2>
      </div>
    </div>
  </div>
  <?php 
    switch( $pg_type ){
      case '':
      case 'all-ebc-events':
      case 'event': ebc_dir_render('stat-leaderboard','by-event', '', $arg = null); break;
    }
  ?>
  <ul style="background-color:#808080" class="accordion xlarge-margin-top" data-accordion data-allow-all-closed="true">
