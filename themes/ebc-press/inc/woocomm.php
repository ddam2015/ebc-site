...
// 	End add multiple products
  ...
  //woocommerce wrappers
	...
	function ebc_wrapper_start() {
		echo '<section id="content" class="grid-x site-main woocomm-wrap" role="main">';
		echo '<div class="cell small-12">';
	}
	function ebc_wrapper_end() {
		echo '</div>';
		echo '</section>';
	}
	add_action('woocommerce_before_main_content', 'ebc_wrapper_start', 10);
	add_action('woocommerce_after_main_content', 'ebc_wrapper_end', 10);
	...
	function ebc_woocommerce_image_dimensions() {
		global $pagenow;
	...
	add_action( 'after_switch_theme', 'ebc_woocommerce_image_dimensions', 1 );

	//remove the breadcrumb (on product pages)
	function ebc_remove_wc_breadcrumbs() {
			remove_action( 'woocommerce_before_main_content', 'woocommerce_breadcrumb', 20, 0 );
	}
	add_action( 'init', 'ebc_remove_wc_breadcrumbs' );
  ...
  add_action( 'woocommerce_before_cart', 'woocommerce_checkout_login_form' );
	...
	function ebc_place_order_button_text() { return __( 'Complete Registration', 'woocommerce' ); }
	add_filter( 'woocommerce_order_button_text', 'ebc_place_order_button_text' );
  ...
  //change add to cart button text
	function ebc_woocommerce_product_add_to_cart_text(){
		return 'Register Now';
	}
	add_filter( 'woocommerce_product_add_to_cart_text' , 'ebc_woocommerce_product_add_to_cart_text' );
	add_filter( 'woocommerce_product_single_add_to_cart_text', 'ebc_woocommerce_product_add_to_cart_text' );
	add_filter( 'woocommerce_booking_single_add_to_cart_text', 'ebc_woocommerce_product_add_to_cart_text' );
...
function ebc_wc_scripts_styles() {
	if( is_admin() ) return;
...
add_action( 'wp_enqueue_scripts', 'ebc_wc_scripts_styles', 100);
...
function g365_add_event_id_reference() {
	wp_enqueue_style( 'foundation-admin', site_url( '/wp-content/themes/ebc-press/css/', 'https' ) . "style-admin.css?ver=2.4" );
...
function ebc_cart_process(){
  ...
  if( !empty(WC()->cart->get_cart()) ) {
    ...
    foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
      ...
      $extra_types = array( 'fee', 'gear', 'event' );
              ...
              for ($i = 1; $i <= $cart_item['quantity']; ++$i) {
                ...
                $extra_fields[] = array(
                  ...
                  'options' => array(
                    ...
                    'y_md' => 'Youth Medium',
                    'y_lg' => 'Youth Large',
                    'a_xs' => 'Adult X-Small / Youth X-Large',
                    'a_sm' => 'Adult Small',
                    'a_md' => 'Adult Medium',
                    'a_lg' => 'Adult Large',
                    'a_xl' => 'Adult X-Large',
     ...
     $extra_fields[] = array(
      'type'  => 'select',
      'placeholder'=> 'Players Jersey Size',
      'label' => 'Jersey Sizing Guide',
      'name'  => 'player_jersey_size_' . $i,
      'required'=> true,
      'class' => array('medium-margin-bottom','jersey-help'),
      'options' => array(
        ''     => 'Select jersey size*',
        'y_sm' => 'Youth Small',
        'y_md' => 'Youth Medium',
...
add_filter( 'woocommerce_before_cart' , 'ebc_cart_setup');
function ebc_cart_setup() {
  //load in global proc var
  global $cart_parse;
  //process cart items
  $cart_parse = ebc_cart_process();
  ...
  if(!empty($cart_parse[2]) ){
    ...
    add_filter( 'woocommerce_after_order_notes' , function( $fields ) use ( $order_data_field ) { return ebc_player_checkout_field( $field, $order_data_field ); });
  }
}

//make the admin credential string
function g365_make_admin_key() {
  $grassroots_keys = get_option( 'ebc_g365_connector' );
  $current_user = wp_get_current_user();
...
function g365_order_data_form_field($no_parameter, $key, $args, $value) {
  ...
  foreach( $args['g365_cart_types'] as $dex => $type ){
    switch( $type ){
      case 'camps':
        $current_user = wp_get_current_user();
//         $presets[] = $type . '_preset,user_ac:EBD-2,user_em:' . $current_user->user_email;
//         $presets[] = $type . '_preset,user_ac:' . ((strpos(site_url(), 'dev') === false) ? 'EBP' : 'EBD') . '-' . $current_user->ID . ',user_em:' . $current_user->user_email;
        $presets[] = $type . '_preset:::user_ac::' . ((strpos(site_url(), 'dev') === false) ? 'EBP' : 'EBD') . '-' . $current_user->ID;
        break;
    }
//     rosters_teams_preset,event_id:351
  }
  if($presets) $presets = 'data-g365_init_pre="' . implode('|', $presets) . '"';
  //return string
  $field = '<div class="grid-x grid-margin-x small-margin-bottom" id="event_details"><div class="cell small-12 medium-12 large-12"><header class="entry-header"><h2 class="entry-title">' . __('Registration') . '</h2></header><div id="g365_registration_fields">';
...
function ebc_player_checkout_field( $checkout, $extra_fields = null ) {
  ...
  echo '<input type="hidden" name="ebc_extra_data_required" value="' . implode(',',$added_field_names_required) . '" />';
  echo '<input type="hidden" name="ebc_extra_data" value="' . implode(',',$added_field_names) . '" />';
}

//make the extra data required based on the field names variable
add_action('woocommerce_checkout_process', 'ebc_player_checkout_field_process');
function ebc_player_checkout_field_process() {
  // Check if we have extra data
  if ( $_POST['ebc_extra_data_required'] ) {
    //parse and loop through the set to make sure we have all the fields filled out
    $fields = explode(',', $_POST['ebc_extra_data_required']);
...
add_action( 'woocommerce_checkout_update_order_meta', 'ebc_player_checkout_field_update_order_meta' );
function ebc_player_checkout_field_update_order_meta( $order_id ) {
  //join the extra_data list if we need to
  $ebc_extra_data = array();
  $ebc_extra_data_fields = array();
  if ( $_POST['ebc_extra_data'] ) {
    $ebc_extra_data[] = $_POST['ebc_extra_data'];
    $ebc_extra_data_fields[] = $_POST['ebc_extra_data'];
  }
  if ( $_POST['ebc_extra_data_required'] ) {
    $ebc_extra_data[] = $_POST['ebc_extra_data_required'];
    $ebc_extra_data_fields[] = $_POST['ebc_extra_data_required'];
  }
  //save the field reference so we know what to pull later
  if ( !empty($ebc_extra_data) ) update_post_meta( $order_id, 'ebc_extra_data', sanitize_text_field( implode(',', $ebc_extra_data) ) );
  // Check if we have extra data
  if ( !empty($ebc_extra_data_fields) ) {
    //parse extra field names, then loop and save
    $fields = explode(',', implode(',', $ebc_extra_data_fields));
    foreach( $fields as $dex => $key ) {
...
add_action( 'woocommerce_admin_order_data_after_billing_address', 'ebc_player_checkout_field_display_admin_order_meta', 10, 1 );
function ebc_player_checkout_field_display_admin_order_meta($order){
  //parse the field names, if we have them, then echo them
  $fields = get_post_meta( $order->id, 'ebc_extra_data', true );
  if( !empty($fields) ) {
...
function ebc_edit_memberships_actions( $actions ) {
...
add_filter( 'wc_memberships_members_area_my-memberships_actions', 'ebc_edit_memberships_actions' );
add_filter( 'wc_memberships_members_area_my-membership-details_actions', 'ebc_edit_memberships_actions' );
    ...
    foreach ( $product_ids as $product_id ) {
      ...
      if( !empty($series_events[ $product_id ][ 'data_pull' ]) ) {
        ...
        if( empty($series_events[ $product_id ][ 'data_pull' ]->dates) ) {
          ...
          //add to the series order so we can be organized
          $series_order[ $product_id ] = date( 'Y-m-d', strtotime(ebc_build_dates($series_events[ $product_id ][ 'data_pull' ]->dates, 4)));
          ...
          if( $payment_options ) {
            ...
            $series_events[ $product_id ][ 'label' ][] = ebc_build_dates($series_events[ $product_id ][ 'data_pull' ]->dates, 1, true);
...
function add_add_upsell_options() {
  ...
  if( !empty( $product_ids ) && is_array($product_ids) ) {
    foreach ( $product_ids as $product_id ) {
      ...
      $add_upsell_description = $add_upsell_product->get_short_description();
...
add_filter( 'woocommerce_checkout_fields' , 'ebc_modify_checkout_fields' );
// Our hooked in function - $fields is passed via the filter!
function ebc_modify_checkout_fields( $fields ) {
   unset($fields['order']['order_comments']);
   return $fields;
}

//validate the checkout form before finishing the process of writting data to ebc about the purchase
add_action('woocommerce_after_checkout_validation', 'process_ebc_data');
function process_ebc_data($form_data) {
...
add_action( 'woocommerce_checkout_update_order_meta', 'ebc_update_meta_form_fields' );
function ebc_update_meta_form_fields( $order_id ) {
...
function ebc_checkout_field_display_admin_order_meta($order){
  ...
  $fields = get_post_meta( $order->get_id(), 'ebc_extra_data', true );
  $message = '<p>No additional order data to retrieve.</p>';
  
  //getting extra products and price of ebc event
  $playerFilm;
  $photoPackage;
  $gear;
  $ebcprice = [];
  foreach ($order->get_items() as $item_id => $item) {
    if($item->get_name() == 'Player Film'){
      $product = $item->get_product();
      $playerFilm = $product->get_price() / $item->get_quantity();
    } else if($item->get_name() == 'Photo Package'){
      $product = $item->get_product();
      $photoPackage = $product->get_price() / $item->get_quantity();
    }else if($item->get_name() == 'VIP Gear Package'){
       $product = $item->get_product();
       $gear = $product->get_price() / $item->get_quantity();
    }
    $product = $item->get_product();
    array_push($ebcprice, $item->get_total()/$item->get_quantity());
  }
...
//                 echo ('grade ' . $val . " ");
                ...
                break;
              case 'height':
                $val = ((!empty($stat_data->height_ft)) ? $stat_data->height_ft . "'" . ((!empty($stat_data->height_in)) ? " " . $stat_data->height_in . '"' : '') : '--');
//                 echo ('height ' . $val . " ");
                array_push($playerVar, $val);
                break;
              default:
...
//                 echo ('testing ' . $val . " ");
          ...
          $parent_name = $order->get_billing_first_name() . ' ' . $order->get_billing_last_name();
          array_push($playerVar, $parent_name);
          array_push($playerVar, $order->get_billing_email());
          array_push($playerVar, $order->get_billing_phone());
          array_push($playerVar, end($ebcprice));
          $coupon = $order->get_used_coupons(); 
          array_push($playerVar, $coupon[0]);
          array_push($playerVar, $playerFilm);
          array_push($playerVar, $photoPackage);
          array_push($playerVar, $gear);
        ...
        continue;
         ...
         array_push($titlesVar, 'Parent');
         array_push($titlesVar, 'Email');
         array_push($titlesVar, 'Phone');
         array_push($titlesVar, 'Registered');
         array_push($titlesVar, 'Code');
         array_push($titlesVar, 'Player Film');
         array_push($titlesVar, 'Photo');
         array_push($titlesVar, 'Gear');
    
  $testing = array_chunk($playerVar, 16);
  

//to display parent info all testing 
//$parent_name .= ' ' . $order->get_billing_last_name();
//print_r($parent_name);
//print_r($order->get_billing_email());
//print_r($order->get_billing_phone());
...
// print_r($playerVar);
  ...
  if(array_key_exists('csvdownload', $_POST)) {
            downloadCSV($titlesVar, $testing, $order->id);
        }
...
//   print_r($exporting);
  ...
  fputcsv($fp, $titleVar, $seperator);
  foreach($playerVar as $lists){
...
function massexportdownloadCSV($titleVar, $playerVar, $orderID){
//   echo 'console.log("yoyo");';
...
//   array_push($exporting, $titleVar);
  ...
  $fp = fopen( 'php://output', 'w');
  $exporting = array_chunk($exporting, 16);
//   print_r($exporting);
    
    fputcsv($fp, $titleVar[0], $seperator);
  foreach($exporting as $lists){
    foreach($lists as $lists2){
      foreach($lists2 as $lists3){
        foreach($lists3 as $lists4){
            fputcsv($fp, $lists4, $seperator);
        }
      }
    }
  }
...
add_filter('bulk_actions-edit-shop_order', function($bulk_actions) {
	$bulk_actions['export-csv'] = __('Export CSV', 'txtdomain');
	return $bulk_actions;
});
...
//     custom start
    $titleVarFinale = array();
    $playerVarFinale = array();
    $all_ids = implode(',', $post_ids);
    foreach($post_ids as $order){      
      $order_data_meta = get_post_meta( $order, '_order_data', true );
      $fields = get_post_meta( $order, 'ebc_extra_data', true );
  ...
  foreach ($order->get_items() as $item_id => $item) {
    if($item->get_name() == 'Player Film'){
      $product = $item->get_product();
      $playerFilm = $item->get_total() / $item->get_quantity();
    } else if($item->get_name() == 'Photo Package'){
      $product = $item->get_product();
      $photoPackage = $item->get_total() / $item->get_quantity();
    }else if($item->get_name() == 'VIP Gear Package'){
       $product = $item->get_product();
       $gear = $item->get_total() / $item->get_quantity();
    }
    $product = $item->get_product();
    array_push($ebcprice, $item->get_total()/$item->get_quantity());
  }
    ...
    //       $message .= '<h3>' . ucfirst($type) . '</h3>';
          if( count($data) > 0 ) {
            ...
            foreach( $order_data_meta_proc[$type]['data'] as $stat_id => $stat_data ) {
              foreach( $column_order as $key => $title ) {
               ...
               $full_order = wc_get_order( $order );
               $parent_name = $full_order->get_billing_first_name() . ' ' . $full_order->get_billing_last_name();
               array_push($playerVar, $parent_name);
               array_push($playerVar, $full_order->get_billing_email());
               array_push($playerVar, $full_order->get_billing_phone());
               array_push($playerVar, end($ebcprice));
               $coupon = $full_order->get_used_coupons(); 
               array_push($playerVar, $coupon[0]);
               array_push($playerVar, $playerFilm);
               array_push($playerVar, $photoPackage);
               array_push($playerVar, $gear);
        ...
        $testing = array_chunk($playerVar, 16);
         ...
         array_push($titlesVar, 'Photo');
         array_push($titlesVar, 'Gear');
         array_push( $titleVarFinale, $titlesVar);
         array_push( $playerVarFinale, $testing);
      
    }
    
    $title = "masspull";
    massexportdownloadCSV($titleVarFinale, $playerVarFinale, $title);
    
//     custom end
		$redirect_url = add_query_arg('export-csv', count($post_ids), $redirect_url);
	}
...
// OLD **** display and edit the custom data
// function OLD_ebc_checkout_field_display_admin_order_meta($order){
//   //for setting form restrictions/helpers
//   $ebc_cart_items = array();
//   //return string
//   $to_write = '';
//   //get items in the order
//   $line_items = $order->get_items( apply_filters( 'woocommerce_admin_order_item_types', 'line_item' ) );
//   //compose object to build form
...
//     //product variation name, if there isn't one it should be empty
//     $product_var_name = ($product_var_id === 0 || method_exists($item, 'get_variation_attributes') === false) ? '' : $item->get_variation_attributes();
//     //get the event id to pull in any further restictions and key the new data
//     $product_event_link = intval(get_post_meta( $product_id, '_event_link', true ));
//     if( !empty($product_event_link) ) {
//       $product_event = get_post_meta( $product_id, '_event_link_data', true );
...
//           $product_div_reorder = (object) [];
//           foreach($product_event_divisions_base as $val) {
//             $present_divs = (isset($level->$val)) ? $level->$val : null;
//             if(!is_null($present_divs)) $product_div_reorder->$val = $present_divs;
//           }
//           $product_event_divisions->$dex = $product_div_reorder;
//         }
...
//     $form_type_target = $form_type_data[1];
//     //product_cat and product_id is needed to organize the cart item data,or if the cat type is invalid
//     if( empty($product_cat_slug) || empty($product_cat_id) || empty($product_id) || empty($form_type_opt[$product_cat_slug])) continue;
//     //add type to types array;
//     $g365_cart_types[] = $form_type_opt[$product_cat_slug];
//     //start the category if there isn't one.
//     if( empty($g365_cart_items[ $product_cat_id ]) ) $g365_cart_items[ $product_cat_id ] = array( 'name' => $product_cat_name, 'title' => ($product_cats[0]->description), 'type' => ($form_type_opt[$product_cat_slug]), 'target' => ($form_type_target[$product_cat_slug]), 'items' => array() );
...
//       'id' => $product_id,
//       'name' => ((empty($product_event_divisions_pull->name)) ? $product_name : $product_event_divisions_pull->name),
//       'full_name' => ((empty($product_event_divisions_pull->short_name)) ? $product_name : $product_event_divisions_pull->short_name),
//       'var_id' => $product_var_id,
//       'var_name' => $product_var_name,
//       'sku' => $product->get_sku(),
//       'qty' => $item->get_quantity(),
//       'event_id' => $product_event_link,
//       'event_id_pm' => $product_event_link,
//       'event_id_ct' => $product_event_link,
//       'event_divisions' => ($product_event_link === 0) ? 0 : $product_event_divisions
//     );
//   }
...
//       // update functionality
//       //
//   wp_enqueue_style( 'foundation-admin', site_url( '/wp-content/themes/ebc-press/css/', 'https' ) . "style-admin.css?ver=2.345" );
// 	wp_enqueue_script( 'admin-jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/2.2.4/jquery.min.js', array(), '2.2.4', true );
//   wp_enqueue_script( 'js-g365-all', (( strpos(site_url(), 'dev.') === false ) ? 'https://grassroots365.com/data-processor.js' : 'https://dev.grassroots365.com/data-processor.js'), array('admin-jquery'), '695450', true );

//   $admin_string = '';
//   if(is_admin()) {
//     $admin_string = ', "g365_admin" : "true"';
//     $order_user = $order->get_user();
//     $admin_string .= ', "g365_user_name" : "' . (( $order_user->user_firstname == '' && $order_user->user_lastname == '' ) ? $order_user->display_name : $order_user->user_firstname . ' ' . $order_user->user_lastname) . '"';
//     $admin_string .= ', "g365_user_email" : "' . $order_user->user_email . '"';
//   }
//   $order_data_meta = get_post_meta( $order->get_id(), '_order_data', true );
//   $admin_key = g365_make_admin_key();
//   echo '</div><div class="clear"></div><div>';
//   echo '<div id="g365_data_manager_admin" class="g365_data_manager_content_wrapper g365_manage_wrapper">';
//   echo '<h3>Additional Order Information</h3>';
//   echo '<script type="text/javascript">var g365_form_details = {"items" : ' . json_encode($g365_cart_items) . $admin_string . ', "wrapper_target" : "g365_form_options_anchor", "user_org": "' . get_user_meta($order_user->ID, '_default_org', true) . '", "admin_key": "' . $admin_key . '"};</script><div><div id="g365_form_options_anchor" data-g365_type="' . get_post_meta( $order->id, '_order_data', true ) . '"></div></div>';
//   echo '<input type="hidden" id="g365_order_data" name="order_data" value="' . $order_data_meta . '" data-g365_og_val="' . $order_data_meta . '" />';
//   echo '</div>';
...
function get_stock_status_text( $product, $name, $term_slug ){
  foreach ( $product->get_available_variations() as $variation ){
    if($variation['attributes'][$name] == $term_slug ) {
      ...
      $in_stock = ( $in_stock === '' ) ? ( ( is_integer($variation['max_qty']) ) ? ' - ' . $variation['max_qty'] . ' left' : '' ) : $in_stock;
      break;
    }
  }
  return $in_stock;
...
add_action( 'show_user_profile', 'ebc_user_fields_display' );
add_action( 'edit_user_profile', 'ebc_user_fields_display' );

function ebc_user_fields_display( $user ) {
	wp_enqueue_style( 'foundation-admin', site_url( '/wp-content/themes/ebc-press/css/', 'https' ) . "style-admin.css?ver=2.345" );
	wp_enqueue_script( 'admin-jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/2.2.4/jquery.min.js', array(), '2.2.4', true );
  wp_enqueue_script( 'js-g365-all', (( strpos(site_url(), 'dev.') === false ) ? 'https://sportspassports.com/data-processor.js' : 'https://dev.sportspassports.com/data-processor.js'), array('admin-jquery'), '695450', true );
  ?>
	<h3><?php esc_html_e( 'Additional User Data', 'ebc' ); ?></h3>
  <div class="options_group g365_manage_wrapper">
...
add_action( 'user_profile_update_errors', 'ebc_user_field_error', 10, 3 );
function ebc_user_field_error( $errors, $update, $user ) {
	if ( ! $update ) {
		return;
	}
}

add_action( 'personal_options_update', 'ebc_user_fields_update' );
add_action( 'edit_user_profile_update', 'ebc_user_fields_update' );

function ebc_user_fields_update( $user_id ) {
