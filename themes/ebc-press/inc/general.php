...
function pl_waiver_columns( $columns ) {
  ...
  $columns['ebc_user_name'] = 'User Name';
  $columns['ebc_user_email'] = 'User Email';
  return $columns;
}
add_filter( 'manage_nf_sub_posts_columns', 'pl_waiver_columns' );

function pl_waiver_column( $column, $sub_id ) {
  if( $column === 'ebc_user_name' || $column === 'ebc_user_email' ) {
    //get submission data
    $sub = Ninja_Forms()->form()->get_sub( $sub_id );
    //pull user info
    $user = $sub->get_user();
    //add it in to the column data
    if( $column === 'ebc_user_name') echo $user->data->user_nicename;
    if( $column === 'ebc_user_email') echo $user->data->user_email;
  }
}
...
function add_apple_touch_icons(){
	...
	echo '<link rel="icon" type="image/png" href="' . get_template_directory_uri() . '/assets/favicon.png" />' . "\n";
...
if ( ! function_exists( 'theme_setup' ) ) :
    ...
    add_theme_support( 'align-wide' );
		...
		load_theme_textdomain( 'ebc-press' );
...
if ( ! function_exists( 'ebc_main_nav' ) ) {
	function ebc_main_nav() {
    ...
    wp_nav_menu( array(
			...
			'fallback_cb'    => false,
			'walker'         => new ebc_Top_Bar_Walker()
		));
	}
}
// Main Navigation Mega
if ( ! function_exists( 'ebc_mega_nav' ) ) {
	function ebc_mega_nav() {
		wp_nav_menu( array(
			'theme_location' => 'main_nav',
			'container'      => false,
			...
			'fallback_cb'    => false,
			'walker'         => new ebc_Mega_Walker()
		));
	}
}
// Main Navigation side Slide
if ( ! function_exists( 'ebc_side_slide_nav' ) ) {
	function ebc_side_slide_nav() {
		wp_nav_menu( array(
			'theme_location' => 'main_nav',
			'container'      => false,
			...
			'fallback_cb'    => false,
			'walker'         => new ebc_Side_Slide_Walker()
		));
	}
}
// Title Nav
if ( ! function_exists( 'ebc_title_nav' ) ) {
	function ebc_title_nav() {
		wp_nav_menu( array(
			'theme_location' => 'title_nav',
			'container'      => false,
...
// Footer Nav
if ( ! function_exists( 'ebc_footer_nav' ) ) {
	function ebc_footer_nav() {
		wp_nav_menu( array(
...
if ( ! function_exists( 'ebc_excerpt' ) ) :
	/**
	 * Displays the optional excerpt.
	 * Wraps the excerpt in a div element.
	 * Create your own ebc_excerpt() function to override in a child theme.
	 * @param string $class Optional. Class string of the div element. Defaults to 'entry-summary'.
	 */
	function ebc_excerpt( $class = 'entry-summary' ) {
...
if ( ! function_exists( 'ebc_excerpt_more' ) && ! is_admin() ) :
/**
 * Replaces "[...]" (appended to automatically generated excerpts) with ... and a 'Continue reading' link.
 * Create your own ebc_excerpt_more() function to override in a child theme.
 * @return string 'Continue reading' link prepended with an ellipsis.
 */
function ebc_excerpt_more() {
	$link = sprintf( '<a href="%1$s" class="more-link">%2$s</a>',
		esc_url( get_permalink( get_the_ID() ) ),
		/* translators: %s: Name of current post */
		sprintf( __( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'ebc-press' ), get_the_title( get_the_ID() ) )
	);
	return ' &hellip; ' . $link;
}
add_filter( 'excerpt_more', 'ebc_excerpt_more' );
endif;

if ( ! function_exists( 'ebc_entry_meta' ) ) :
/**
 * Prints HTML with meta information for the categories, tags.
 * Create your own ebc_entry_meta() function to override in a child theme.
 */
function ebc_entry_meta() {
	if ( 'posts' === get_post_type() ) {
		$author_avatar_size = apply_filters( 'ebc_author_avatar_size', 49 );
		printf( '<span class="byline"><span class="author vcard">%1$s <span class="screen-reader-text">%2$s </span> <a class="url fn n" href="%3$s">%4$s</a></span></span>',
			get_avatar( get_the_author_meta( 'user_email' ), $author_avatar_size ),
			_x( 'Author', 'Used before post author name.', 'ebc-press' ),
	...
	if ( current_theme_supports( 'post-formats', $format ) ) {
		printf( '<span class="entry-format">%1$s<a href="%2$s">%3$s</a></span>',
			sprintf( '<span class="screen-reader-text">%s </span>', _x( 'Format', 'Used before post format.', 'ebc-press' ) ),
	...
	if ( 'posts' === get_post_type() ) {
// 		ebc_entry_taxonomies();
	}

	if ( ! is_singular() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( sprintf( __( 'Leave a comment<span class="screen-reader-text"> on %s</span>', 'ebc-press' ), get_the_title() ) );
		echo '</span>';
	}
}
endif;

if ( ! function_exists( 'ebc_entry_date' ) ) :
/**
 * Prints HTML with date information for current post.
 * Create your own ebc_entry_date() function to override in a child theme.
 */
function ebc_entry_date() {
...
// 		$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time> | <time class="updated" datetime="%3$s">%4$s</time>';
	...
	printf( '<span class="posted-on"><span class="screen-reader-text">%1$s </span><a href="%2$s" rel="bookmark">%3$s</a></span>',
		_x( 'Posted on', 'Used before publish date.', 'ebc-press' ),
...
function ebc_admin_menu() {
	add_menu_page( 'OGP Data Admin', 'Data', 'manage_options', 'ebc_data', 'ebc_admin', 'dashicons-chart-line', 26  );
// 	add_submenu_page( 'ebc_data', 'ebc Data Dashboard', 'Dashboard', 'manage_options', 'ebc_data', 'ebc_admin' );
}
add_action( 'admin_menu', 'ebc_admin_menu' );

//build admin page
function ebc_admin(){
	wp_enqueue_style( 'ebc_admin_styles', get_stylesheet_directory_uri() . '/css/ebc-admin-style.css' );
	...
	$ebc_sections = array(
		'settings' => array(
			...
			'tab_name'	=> 'Settings',
			'url'			=> '?page=ebc_data&g_action=settings'
		)
	);
	echo '<div class="ebc_data_manager_wrapper">';
	$ebc_admin_url = '';
	$nav = '';
	foreach( $ebc_sections as $section => $section_data ) {
		if( $section == $g_action ) {
			echo '<h1>' . $section_data['title'] . '</h1>';
			$ebc_admin_url = $section_data['url'];
	...
	echo '<nav class="nav-tab-wrapper">' . $nav . '</nav><div class="ebc_data_manager_content_wrapper">';
	switch( $g_action ) {
		case 'export_data':
			ebc_export();
			break;
		...
		case 'settings':
			...
			function ebc_update_admin_site_settings( $old, $new ) {
				foreach( $new as $item => $item_data ) {
					foreach( $item_data as $item_name => $item_value ) {
						$old[$item]['items'][$item_name]['value'] = ebc_process_data_point( $old[$item]['items'][$item_name]['type'], $item_value);
            if( $old[$item]['items'][$item_name]['value'] === null || $old[$item]['items'][$item_name]['value'] === 'null' ) $old[$item]['items'][$item_name]['value'] = '';
					}
				}
				return $old;
			}
			foreach( $ebc_admin_settings as $setting_set => &$setting_data ) {
				if( $_POST['g_process'] == 'process' ) {
					$ebc_db_option = $_POST['ebc_admin_form_data'][$setting_set];
					$update = update_option( 'ebc_' . $setting_set, $ebc_db_option );
					$setting_data['section_records'] = ebc_update_admin_site_settings( $setting_data['section_records'], $ebc_db_option );
					if( $update === true ) echo '<p class="success">Updated settings data!</p>';
				} else {
					$ebc_db_option = get_option( 'ebc_' . $setting_set );
					if( $ebc_db_option !== false && !empty($ebc_db_option) ) $setting_data['section_records'] = ebc_update_admin_site_settings( $setting_data['section_records'], $ebc_db_option );
        }
			} ?>
			<h3>General Site Settings</h3>
			<form id="ebc_form" method="post" class="ebc_form" action="<?php echo $ebc_admin_url; ?>">
				<input type="hidden" name="g_process" value="process" />
			<?php foreach( $ebc_admin_settings as $setting_set_re => $setting_data_re ) : ?>
				...
				<table class="ebc_form_section form-table">
					<tbody>
					<?php foreach( $record_data['items'] as $element => $element_data ) {
						$element_data['tag'] = 'ebc_admin_form_data[' . $setting_set_re . '][' . $record . '][' . $element . ']';
						$element_data['description'] = ( empty($element_data['description']) ) ? '' : '<small>' . $element_data['description'] . '</small>';
						echo ebc_template_construction( $element_data );
					} ?>
					</tbody>
				</table>
				<?php endforeach;
...
function send_html_email($email, $subject, $message) {
  $headers = ( strpos( site_url(), 'dev' ) !== false ) ? array('From: EBC Dev Customer Service <no-reply@dev.elitebasketballcircuit.com>') : array('From: EBC Customer Service <no-reply@elitebasketballcircuit.com>');
...
// 	$pageID = ( $pageID === null ) ? $post->ID : $pageID;
	...
	$ebc_site_ads = get_option( 'ebc_display' );
	//if the page doesn't have an ad or the site global is empty, don't add to array
	$ad_info['go'] = ( empty($ebc_page_ad['link']) || empty($ebc_page_ad['img']) || empty($ebc_site_ads) ) ? false : true;
	...
	if ( $ad_info['go'] ) {
		...
		$ebc_site_ads['site_1']['element_type'] = 'rotator';
		$ebc_site_ads['site_2']['element_type'] = 'rotator';
		//build rotator element for page ad
		$ad_info['ad_content'] .= ebc_template_construction( $ebc_page_ad );
		//if we have a global ad, build and add it
		if( !empty($ebc_site_ads['site_1']['img']) ) $ad_info['ad_content'] .= ebc_template_construction( $ebc_site_ads['site_1'] );
		//if we have a secondary global ad, build if and add it along with another primary ad to help the flow
		if( !empty($ebc_site_ads['site_2']['img']) ) {
			$ad_info['ad_content'] .= ebc_template_construction( $ebc_page_ad );
			$ad_info['ad_content'] .= ebc_template_construction( $ebc_site_ads['site_2'] );
		}
	}
	//on the front page, and if we have a global splash setting
	if( is_front_page() && !empty($ebc_site_ads['site_3']) ) {
		//controls for the splash
		$ebc_splash = array(
			'title'				=> ( empty($ebc_site_ads['site_3']['title']) ) ? '' : $ebc_site_ads['site_3']['title'],
			'link'				=> ( empty($ebc_site_ads['site_3']['link']) ) ? '' : $ebc_site_ads['site_3']['link'],
			'img'					=> ( empty($ebc_site_ads['site_3']['img']) ) ? '' : $ebc_site_ads['site_3']['img'],
			'img_mobile'	=> ( empty($ebc_site_ads['site_3']['img_mobile']) ) ? '' : $ebc_site_ads['site_3']['img_mobile'],
			'description'	=> ( empty($ebc_site_ads['site_3']['description']) ) ? '' : $ebc_site_ads['site_3']['description']
		);
		//if we have minimum info process the splash
		if( !empty($ebc_splash['title']) && !empty($ebc_splash['link']) && !empty($ebc_splash['img']) ) {
			//build the splash html
			$ad_info['splash'] = '<div class="reveal text-center" id="ebc_home_reveal" aria-labelledby="' . $ebc_splash['title'] . '" data-reveal><div class="relative"><h1 id="reveal-title" class="show-for-sr">' . $ebc_splash['title'] . '</h1>';
			//if there is a derscription, add it
			if( !empty($ebc_splash['description']) ) $ad_info['splash'] .= '<p class="reveal-description">'. $ebc_splash['description'] . '</p>';
			//if there is a mobile image, add it
			if( !empty($ebc_splash['img_mobile']) ) $ad_info['splash'] .= '<p class="text-center show-for-small-only"><a href="' . $ebc_splash['link'] . '"><img src="'. $ebc_splash['img_mobile'] . '" alt="'. $ebc_splash['title'] . '" title="'. $ebc_splash['title'] . '" /></a></p>';
			//if there is a mobile image, add supporting class to the large image
			$mobile_img_class = ( empty($ebc_splash['img_mobile']) ) ? '' : ' show-for-medium';
			//add main splash image
			$ad_info['splash'] .= '<p class="text-center' . $mobile_img_class . '"><a href="' . $ebc_splash['link'] . '"><img src="'. $ebc_splash['img'] . '" alt="'. $ebc_splash['title'] . '" title="'. $ebc_splash['title'] . '" /></a></p>';
...
function ebc_template_construction( $ele_data ) {
	$ebc_admin_form_elements = array(
		'input' 	=> array(
			'template' => '<tr>
					<th>
	...
	$ele = $ebc_admin_form_elements[$ele_data['element_type']]['template'];
	foreach( $ebc_admin_form_elements[$ele_data['element_type']]['vars'] as $dex => $var ) {
    if( $var === 'options' ) {
      $all_options = '';
      $option_ele = $ebc_admin_form_elements[$ele_data['element_type']]['template_option'];
...
function ebc_process_data_point( $name, $point ) {
...
function ebc_build_dates($dates, $type = 1, $abbv = false, $add_reg = false) {
    ...
    switch( $type ){
      ...
      case 'date':
        ...
        if( empty($event_object->dates) ) {
          ...
          $date_reg = ebc_build_dates($event_object->dates, 2, false, $reg_days);
          if( $reg_days === false ) {
      ...
      case 'full_address':
        if( empty($event_object->short_locations) ) {
          ...
          $reg_days = $atts['reg_days'];
          $date_reg = ebc_build_dates($event_object->short_locations, 2, false, $reg_days);
          if( $reg_days === false ) {
            $data_compile .= $date_reg;
          } else {
            $data_compile .= $date_reg[0];
...
function g365_player_spotlight_build_new( $atts ){
  $player_spotlight_remote_data = g365_fn(['fn_name'=>'g365_remote_api', 'arguments'=>['ebc-watchlist', ['post_id'=>$post->ID, 'wt_id'=>$wp_query->query_vars['wt_id'], 'wt_tp'=>$wp_query->query_vars['wt_tp']]]]);
  $spotlight_data = $player_spotlight_remote_data->stat_leaderboard;
  $alumni_players = array();
  foreach($player_spotlight_remote_data->watchlist_data->records[1]->records as $alumni_data){
      $alumni_players[$alumni_data->ranking_type]['player_name'] =  $player_spotlight_remote_data->watchlist_data->player_records->{$alumni_data->rankings[0]}->name;
      $alumni_players[$alumni_data->ranking_type]['player_img'] =  $player_spotlight_remote_data->watchlist_data->player_records->{$alumni_data->rankings[0]}->player_img;
      $alumni_players[$alumni_data->ranking_type]['player_url'] =  $player_spotlight_remote_data->watchlist_data->player_records->{$alumni_data->rankings[0]}->player_url;
      $alumni_players[$alumni_data->ranking_type]['date'] =  date("F Y", strtotime($player_spotlight_remote_data->watchlist_data->records[0]->records[0]->start_datetime));
  }
  $stat_leaderboards = $spotlight_data->stat_leaderboard;
  $return_string .= '<div class="grid-x grid-margin-x small-up-2 medium-up-4 text-center profile-feature profile-widget player-spotlight">
    <div class="callout gray player-spotlight__section">
      <div class="orbit" role="region" aria-label="Stat Leaderboard" data-orbit>
        <div class="orbit-wrapper">
          ...
          <ul class="orbit-container">';
            foreach($stat_leaderboards as $stat_leaderboard):
              $default_img = 'http://sportspassports.com/wp-content/uploads/event-profiles/ebc_profile_placeholder.png';
              empty($stat_leaderboard->ev_profile_img) ? $profile_img = $default_img : $profile_img = $stat_leaderboard->ev_profile_img;
              $avg_pts = round($stat_leaderboard->avg_stat, 2);
            
            $return_string .= '<li class="is-active orbit-slide">
              <div class="player-spotlight__info">
                <div class="grid-y">
                    <h5 class="weight-bold no-margin-bottom">
                      <a class="spotlight__card--heading" href="https://sportspassports.com/player/'.$stat_leaderboard->player_url.'">'.$stat_leaderboard->player_name.'</a>
                    </h5>
                ...
                <div class="relative small-margin-top small-margin-bottom player-spotlight__pic">
                  <a href="https://sportspassports.com/player/'.$stat_leaderboard->player_url.'">
                    <img class="profile-image__player" src="'.$profile_img.'" alt="'.$profile_img.'">
                  </a>
                </div>
                <a class="player-spotlight__event" href="https://sportspassports.com/event/'.$stat_leaderboard->event_nickname.'" target="_blank">
        ...
        <nav class="orbit-bullets">
          ...
          <button data-slide="4" class=""></button>
        </nav>
      </div>
    </div>
    <div class="callout gray player-spotlight__section">
      <div class="orbit" role="region" aria-label="Stat Leaderboard" data-orbit>
        <div class="orbit-wrapper">
          <h5 class="weight-bold small-margin-bottom player-spotlight__heading">Top Alumni</h5>
          <ul class="orbit-container">';
            foreach($alumni_players as $index => $alumni_player):
              $default_img = 'http://sportspassports.com/wp-content/uploads/event-profiles/ebc_profile_placeholder.png';
              empty($alumni_player['player_img']) ? $profile_img = $default_img : $profile_img = $alumni_player['player_img'];
              $return_string .= '<li class="is-active orbit-slide">
                <div class="player-spotlight__info">
                  <div class="grid-y">
                      <h5 class="weight-bold no-margin-bottom">
                        <a class="spotlight__card--heading" href="https://sportspassports.com/player/'.$alumni_player['player_url'].'">'.$alumni_player['player_name'].'</a>
                      </h5>
                      <h5 class="weight-bold small-margin-bottom  player-spotlight__stat">'.$index.'</h5>
                  </div>
                  <div class="relative small-margin-top small-margin-bottom player-spotlight__pic">
                    <a href="https://sportspassports.com/player/'.$alumni_player['player_url'].'">
                      <img class="profile-image__player" src="'.$profile_img.'" alt="'.$profile_img.'">
                    </a>
                  </div>
                  <a class="player-spotlight__event" href="https://elitebasketballcircuit.com/players-to-watch/#ebctop25alumni">
                    <img class="profile-event-img tiny-margin-top" src="https://elitebasketballcircuit.com/wp-content/uploads/2020/07/EBC_logoGeneral-Lg.png" alt="ebc-logo">'.$alumni_player['date'].'
                  </a>
                </div>
              </li>';
            endforeach;
...
add_shortcode( 'g365_player_splotlight_new', 'g365_player_spotlight_build_new' );
...
add_action('woocommerce_cart_calculate_fees', function(){
  ...
  $percentage_fee = (WC()->cart->get_cart_contents_total() + WC()->cart->get_shipping_total()) * $percentage;
   WC()->cart->add_fee(__('Credit Card Surcharge 2.9%', 'txtdomain'), $percentage_fee);
});
...
function custom_override_checkout_fields( $fields ) {
    ...
    $fields['billing']['billing_company']['placeholder'] = 'Billing Company (optional)';
