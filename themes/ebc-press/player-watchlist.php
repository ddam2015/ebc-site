...
get_header();
global $wp_query;
$watchlist_remote_data = g365_fn(['fn_name'=>'g365_remote_api', 'arguments'=>['ebc-watchlist', ['post_id'=>$post->ID, 'wt_id'=>$wp_query->query_vars['wt_id'], 'wt_tp'=>$wp_query->query_vars['wt_tp']]]]);
// $g365_ad_info = g365_start_ads( $post->ID );
$default_profile_img = 'http://grassroots365.com/wp-content/uploads/event-profiles/ebc_profile_placeholder.png';
  ...
  if( $watchlist_data->groups == 1 ) : ?>
    ...
    <div class="tabs separate grid-x small-up-2 medium-up-3 large-up-7 align-center text-center collapse" id="event-tabs" data-tabs data-deep-link="true" data-deep-link-smudge="true" data-deep-link-smudge-delay="600">
      <?php foreach( $watchlist_data->records as $dex => $group_data ) : if( empty($group_data->item_ids) ) continue; ?>
        <div style="width:17%" class="tabs-title cell<?php echo ( $dex == 0 ) ? ' is-active' : ''; ?>">
        ...
        <div class="relative">
          <div class="watchlist__tabs-wrapper mobileScroll">
              <nav class="tabs separate grid-x small-up-2 medium-up-3 large-up-7 align-center text-center collapse medium-padding-bottom" id="<?php echo $group_data->handle; ?>_players" data-tabs>
                  ...
                  <div class="watchlist__nav tabs-title<?php echo ( $dex == 0 ) ? ' is-active' : ''; ?>"><a style="color:#fff;" href="#<?php echo $group_data->handle; ?>_players_<?php echo $subgroup_data->id; ?>"><?php echo $subgroup_data->ranking_type; ?></a></div>
                  <?php endforeach; ?>
              </nav>
          </div>
            ...
            <?php foreach( $group_data->records as $dex => $subgroup_data ) : if( empty($subgroup_data->rankings) ) continue; ?>
              <div class="watchlist__panel gray-bg tabs-panel<?php echo ( $dex == 0 ) ? ' is-active' : ''; ?>" id="<?php echo $group_data->handle; ?>_players_<?php echo $subgroup_data->id; ?>">
                <div class="grid-x grid-margin-x small-up-2 medium-up-3 large-up-5 align-center text-center img-grid">
                  <?php
                    echo '<h4 class="large-12" style="background-color: #222222; padding: 6px; border-radius: 4px;">Top 10</h4>';
                    foreach( $subgroup_data->rankings as $subdex => $player_id ) :
                      if($subdex == '10') echo '<h4 class="large-12" style="background-color: #222222; padding: 6px; border-radius: 4px;">Top 20</h4>';
                      if($subdex == '20') echo '<h4 class="large-12" style="background-color: #222222; padding: 6px; border-radius: 4px;">Top 50</h4>';
                      if($subdex == '50') echo '<h4 class="large-12" style="background-color: #222222; padding: 6px; border-radius: 4px;">Top 70</h4>';
                      if($subdex == '70') echo '<h4 class="large-12" style="background-color: #222222; padding: 6px; border-radius: 4px;">Top 100</h4>';
                  ...
                  ?>
                    <div class="cell">
                      <a class="emphasis ebc_watchlist__player" href="http://sportspassports.com/player/<?php echo $player_url; ?>" target="_blank">
                        ...
                        <p style="color:#fff;"><?php echo $player_name; ?></p>
                      </a>
                    </div>
                  <?php endforeach; ?>
                </div>
