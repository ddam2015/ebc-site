...
$default_profile_img = get_site_url() . '/wp-content/themes/ebc-press/assets/ebc_profile_placeholder.jpg';
...
<section id="content" class="grid-x grid-margin-x site-main large-padding-top xlarge-padding-bottom<?php if ( $ebc_ad_info['go'] ) echo $ebc_ad_info['ad_section_class']; ?>" role="main">
	<div class="cell small-12">
		...
		<div class="tabs separate grid-x small-up-2 medium-up-3 large-up-4 align-center text-center collapse" id="event-tabs" data-tabs>
			<?php foreach( $award_data->records as $dex => $group_data ) : if( empty(array_intersect($group_data->item_ids, $award_data->data_present)) || $group_data->name === 'EBC San Diego' ) continue; ?>
			...
			<div class="grid-x tabs-panel small-padding<?php echo ( $dex == 0 ) ? ' is-active' : ''; ?>" id="<?php echo $group_data->handle; ?>">
				<div class="cell gset medium-padding">
					...
					<div>
						...
						<?php foreach( $group_data->records as $ev_dex => $event_data ) : if( empty(in_array($event_data->id, $award_data->data_present)) ) continue; ?>
							<div class="tabs-panel alumni-table-container <?php echo ( $ev_dex == 0 ) ? ' is-active' : ''; ?>" id="<?php echo $group_data->handle; ?>_awards_<?php echo date('Y',strtotime($event_data->eventtime)); ?>">
								 <table class="pTable text-center no-margin-bottom scroll-for-small">
									<tbody>
										...
										$col_count = count( $col_classes ); sort($col_classes);
...
// 													echo '&nbsp;';
                          echo '';
													...
													echo $award_vals[$award_class][$i]->player_name . '<span class="camp-class--mobile show-for-small-only">Class of '.$award_vals[$award_class][$i]->award_class .'</span></a>';
												}
												echo '</td>';
											}
											echo '</tr>';
